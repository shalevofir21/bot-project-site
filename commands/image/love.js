...
module.exports = {
	...
	run: async (bot, message, args) => {
		let user =  await [...message.mentions.members.values()][0] || message.guild.members.cache.get(args[0]) || message.guild.members.cache.find(m => m.user.username.toLowerCase() === args[0]?.toLocaleLowerCase()) || message.guild.members.cache.find(mp => mp.displayName.toLowerCase() === args[0]?.toLocaleLowerCase());
		let user2 =  await [...message.mentions.members.values()][1] || message.guild.members.cache.get(args[1]) || message.guild.members.cache.find(m => m.user.username.toLowerCase() === args[1]?.toLocaleLowerCase()) || message.guild.members.cache.find(mp => mp.displayName.toLowerCase() === args[1]?.toLocaleLowerCase());
